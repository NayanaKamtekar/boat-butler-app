...
WHERE LOWER(user.name) = '${req.body.name}' AND user_role_id = ${companyUserRoleID};`;
    ...
    pool.query(selectUser, async (err, rows, fields) => {
      if (err) {
        ...
        res
          .status(201)
          .send(
            `{"user_id": "${rows?.[0]?.user_id}", "user_name": "${rows?.[0]?.user_name}", "company_id": "${rows?.[0]?.company_id}", "is_auth": "${is_auth}"}`
          );
      } else {
